...
appendAll ref strings = do
    ...
    -- Append all strings from the list to the current value
    let newValue = currentValue ++ concat strings
...
doubleCall op = do
    op' <- op  -- Run the first operation to get the second operation
    op'       -- Run the second operation
...
compose op1 op2 c = do
    a <- op2 c  -- Run the second operation with the initial value
    op1 a      -- Run the first operation with the result of the second operation
...
interact' f state = do
    line <- getLine
    let (cont, response, newState) = f (line, state)
    putStrLn response
    if cont
