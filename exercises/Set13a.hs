...
checkCapitals pair@(a, b)
  | isUpper (head a) && isUpper (head b) = Just pair
  | otherwise                             = Nothing
...
countAndLog pred xs = Logger (map show filtered) (length filtered)
...
rob from to = balance from +> \amount -> withdrawOp from amount +>> depositOp to amount
...
paren c = modify $ \s -> if s < 0 then s else case c of
                          '(' -> s + 1
                          ')' -> s - 1
                          _   -> s
...
count x = modify $ \xs -> case lookup x xs of
                            Just n  -> (x, n + 1) : filter ((/= x) . fst) xs
                            Nothing -> (x, 1) : xs
