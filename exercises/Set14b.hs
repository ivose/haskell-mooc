...
import Database.SQLite.Simple (Connection, Query(..), Only(..), open, execute, execute_, query, query_)
...
openDatabase filename = do
  ...
  conn <- open filename
  -- Run the initialization query to create the table if it doesn't exist
...
parseCommand [cmd, account] | cmd == T.pack "balance" = Just $ Balance account
parseCommand [cmd, account, amtText] | cmd == T.pack "deposit" = Deposit account <$> parseInt amtText
                                      | cmd == T.pack "withdraw" = Withdraw account . negate <$> parseInt amtText
...
perform conn (Just (Withdraw account amount)) = do
  deposit conn account amount  -- use negative amount for withdrawal
  return $ T.pack "OK"
