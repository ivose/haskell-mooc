...
oddFactorial 1 = 1
oddFactorial n
  | even n = oddFactorial (n - 1)  -- If n is even, start from n-1
  | otherwise = n * oddFactorial (n - 2)  -- Multiply n by the oddFactorial of n-2
...
leftpad str len
  | len > length str = replicate (len - length str) ' ' ++ str
  | otherwise = str
...
smallestDivisor n = helper 2  -- Start checking from 2
  where
    helper divisor
      | divisor * divisor > n = n 
...
biggestPrimeAtMost n
  | n < 2 = error "Undefined for numbers less than 2"
  | isPrime n = n
