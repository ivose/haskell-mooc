...
interpreter commands = process commands 0 0
  where
    process :: [String] -> Int -> Int -> [String]
    process [] _ _ = []
    process (cmd:cmds) x y =
      case cmd of
        "up"     -> process cmds x (y + 1)
        "down"   -> process cmds x (y - 1)
        "left"   -> process cmds (x - 1) y
        "right"  -> process cmds (x + 1) y
        "printX" -> show x : process cmds x y
        "printY" -> show y : process cmds x y
        _        -> process cmds x y  -- Ignore unrecognized commands
