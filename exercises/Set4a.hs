...
incrementKey targetKey = map incrementIfKeyMatches
  where
    incrementIfKeyMatches (key, value)
      | key == targetKey = (key, value + 1)
      | otherwise = (key, value)
...
transfer from to amount bank
  | amount <= 0 = bank  -- Check if the sum is negative or zero
  | not (Map.member from bank) = bank  -- Check if the 'from' account exists
  | not (Map.member to bank) = bank  -- Check if the 'to' account exists
  | fromBalance < amount = bank  -- Check if the 'from' account has enough money
...
swap :: Ix i => i -> i -> Array i e -> Array i e
swap i j arr
  | i == j = arr  -- No need to swap if indices are the same
  | otherwise = arr // [(i, arr ! j), (j, arr ! i)]
