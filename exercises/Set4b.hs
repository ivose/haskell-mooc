...
largestHelper x [] = [x]  -- If the accumulator is empty, start with the current element.
largestHelper x acc@(y:_)
  | x > y = [x]  -- If the current element is larger, start a new list with it.
  | x == y = x : acc  -- If it's equal to the largest so far, add it to the accumulator.
  | otherwise = acc  -- If it's smaller, keep the accumulator unchanged.
