...
-- Helper function to check if all values in a tree satisfy the condition that they are within a specified range
withinRange :: Ord a => Tree a -> Maybe a -> Maybe a -> Bool
withinRange Empty _ _ = True
withinRange (Node value left right) minBound maxBound =
    withinBounds value minBound maxBound &&
    withinRange left minBound (Just value) &&
    withinRange right (Just value) maxBound
  where
    withinBounds v Nothing Nothing = True
    withinBounds v (Just minV) Nothing = v > minV
    withinBounds v Nothing (Just maxV) = v < maxV
    withinBounds v (Just minV) (Just maxV) = v > minV && v < maxV

isOrdered :: Ord a => Tree a -> Bool
isOrdered tree = withinRange tree Nothing Nothing
...
set [] val (Node _ left right) = Node val left right  -- Replace the value at the current node
set [] val Empty = Empty
set (StepL:steps) val Empty = Empty  -- Path falls off the tree
set (StepR:steps) val Empty = Empty  -- Path falls off the tree
set (StepL:steps) val (Node value left right) = Node value (set steps val left) right  -- Traverse left
set (StepR:steps) val (Node value left right) = Node value left (set steps val right)  -- Traverse right
...
search val (Node value left right)
  ...
  | otherwise = case search val left of
      ...
      Nothing -> fmap (StepR :) (search val right)
