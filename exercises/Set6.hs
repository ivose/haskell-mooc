...
eggPrice = price ChickenEgg 
milkPrice = price (Milk 2) 
...
-- Instance for Maybe a, given that a is an instance of Price
instance Price a => Price (Maybe a) where
  price Nothing = 0
  price (Just item) = price item

-- Instance for list of items, given that the items are instances of Price
...
simplify (RationalNumber numerator denominator) =
    ...
    in RationalNumber (numerator `div` d) (denominator `div` d)
...
class Cycle a where
  ...
  -- Default implementation of stepMany using recursion
  stepMany :: Int -> a -> a
  stepMany 0 x = x
