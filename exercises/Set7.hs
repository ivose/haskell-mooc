...
-- Helper function to insert an element into a list in sorted order without adding duplicates
...
average xs = sum (toList xs) / fromIntegral (length (toList xs))
...
passwordAllowed pwd (DoesNotContain chars) = all (`notElem` chars) pwd
