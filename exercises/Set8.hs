...
combine blendFunc (Picture p1) (Picture p2) =
  Picture (\coord -> blendFunc (p1 coord) (p2 coord))
...
paintSolid color (Shape s) (Picture base) = Picture newPic
  where
    newPic coord = if s coord then color else base coord
...
paint (Picture patternPic) (Shape s) (Picture base) = Picture newPic
  where
    newPic coord = if s coord then patternPic coord else base coord
...
instance Transform Zoom where
  apply (Zoom z) (Picture pic) = Picture (\(Coord x y) -> pic (Coord (x `div` z) (y `div` z)))
...
instance Transform Blur where
  apply Blur (Picture pic) = Picture blurredPic
    where
      blurredPic (Coord x y) = averageColor [
          pic (Coord x y),
          pic (Coord (x-1) y),
          pic (Coord (x+1) y),
          pic (Coord x (y-1)),
...
averageColor colors = Color (avg reds) (avg greens) (avg blues)
  where
    (reds, greens, blues) = unzip3 [(r, g, b) | Color r g b <- colors]
